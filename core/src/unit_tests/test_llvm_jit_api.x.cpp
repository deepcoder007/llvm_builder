...
TEST(LLVM_CODEGEN_JIT_API, hello_api) {
    ...
    CODEGEN_LINE(Module::Context l_module_ctx{l_module})
    {
...

        {
            ...
            CORE_ALWAYS_ASSERT(l_struct.pointer_type().check_sync(ctx));
    ...
    CORE_ALWAYS_ASSERT(not ErrorContext::has_error());
    {
        ...
        CORE_ALWAYS_ASSERT(not l_args.has_error())
        CORE_ALWAYS_ASSERT(not test_fn.has_error())

        CORE_ALWAYS_ASSERT_EQ(l_args.num_fields(), 5);
        ...
        for (int32_t i = 0; i != 10; ++i) {
            ...
            CORE_ALWAYS_ASSERT_EQ(l_args_obj.get<int32_t>("field_1"), i);
            CORE_ALWAYS_ASSERT_EQ(l_args_obj.get<int32_t>("field_2"), i + 1);
            CORE_ALWAYS_ASSERT_EQ(l_args_obj.get<int32_t>("field_3"), i);
            CORE_ALWAYS_ASSERT_EQ(l_args_obj.get<int32_t>("field_4"), i + 1);
...
TEST(LLVM_CODEGEN_JIT_API, inner_struct) {
    ...
    CODEGEN_LINE(Module::Context l_module_ctx{l_module})
    {
...

        {
            ...
            CORE_ALWAYS_ASSERT(l_inner_struct.pointer_type().check_sync(arg_inner));
            CORE_ALWAYS_ASSERT(l_outer_struct.pointer_type().check_sync(arg_outer));
            ...
            for (const std::string& fname : std::initializer_list<std::string>{{"field_1", "field_2", "field_3", "field_4"}}) {
                ...
                CODEGEN_LINE(arg_outer.field(CORE_CONCAT << "inner_" << fname).store(l_input));
            }
    ...
    CORE_ALWAYS_ASSERT(not ErrorContext::has_error());
    {
        ...
        CORE_ALWAYS_ASSERT_EQ(l_args.num_fields(), 2);
        CORE_ALWAYS_ASSERT_EQ(l_outer_struct.num_fields(), 10);
        CORE_ALWAYS_ASSERT_EQ(l_inner_struct.num_fields(), 5);
        CORE_ALWAYS_ASSERT_EQ(l_args_struct.num_fields(), 2);
        ...
        for (int32_t i = 0; i != 10; ++i) {
            ...
            CORE_ALWAYS_ASSERT(l_inner_obj.is_frozen());
            CORE_ALWAYS_ASSERT(l_outer_obj.is_frozen());
            CODEGEN_LINE(l_args_obj.set_object("arg_inner", l_inner_obj))
            CODEGEN_LINE(l_args_obj.set_object("arg_outer", l_outer_obj))
            CODEGEN_LINE(l_args_obj.try_freeze())

            CODEGEN_LINE(test_fn.on_event(l_args_obj))
            CORE_ALWAYS_ASSERT_EQ(l_inner_obj.get<int32_t>("field_1"), l_inner_obj.get<int32_t>("field_3"))
            CORE_ALWAYS_ASSERT_EQ(l_inner_obj.get<int32_t>("field_2"), l_inner_obj.get<int32_t>("field_4"))
            std::vector<std::string> field_list;
            field_list.emplace_back("field_1");
            field_list.emplace_back("field_2");
            for (const std::string& fname : field_list) {
                CORE_ALWAYS_ASSERT_EQ(l_inner_obj.get<int32_t>(fname), l_outer_field_6_obj.get<int32_t>(fname))
            }
            field_list.emplace_back("field_3");
            field_list.emplace_back("field_4");
            for (const std::string& fname : field_list) {
                CORE_ALWAYS_ASSERT_EQ(l_inner_obj.get<int32_t>(fname), l_outer_field_9_obj.get<int32_t>(fname))
            }
            CORE_ALWAYS_ASSERT_EQ(true, l_inner_obj.get<bool>("field_5"));

            RuntimeObject l_outer_field_9_obj_v2 = l_outer_obj.get_object("inner_field_9");
            CORE_ALWAYS_ASSERT(not l_outer_field_9_obj_v2.has_error());
            CORE_ALWAYS_ASSERT_EQ(l_outer_field_9_obj.struct_def(), l_outer_field_9_obj_v2.struct_def());
            CORE_ALWAYS_ASSERT_EQ(l_outer_field_9_obj.ref(), l_outer_field_9_obj_v2.ref());
            CORE_ALWAYS_ASSERT_EQ(l_outer_field_9_obj, l_outer_field_9_obj_v2);
...
TEST(LLVM_CODEGEN_JIT_API, array_basic_1d) {
    ...
    CORE_ALWAYS_ASSERT(not ErrorContext::has_error());
    {
        ...
        CORE_ALWAYS_ASSERT_EQ(l_args.num_fields(), 2);
        CORE_ALWAYS_ASSERT_EQ(l_outer_struct.num_fields(), 12);
        CORE_ALWAYS_ASSERT_EQ(l_inner_struct.num_fields(), 5);
        CORE_ALWAYS_ASSERT_EQ(l_args_struct.num_fields(), 2);
...
TEST(LLVM_CODEGEN_JIT_API, full_test) {
    ...
    CORE_ALWAYS_ASSERT(not ErrorContext::has_error());
    {
        ...
        CORE_ALWAYS_ASSERT_EQ(l_outer_struct.num_fields(), 3);
        ...
        for (int32_t i = 0; i != 1; ++i) {
            ...
            CORE_ALWAYS_ASSERT(l_outer_field_3_obj.is_frozen())
            ...
            for (uint32_t i = 0; i != 5; ++i) {
                CORE_ALWAYS_ASSERT_EQ(src_arr.get<int32_t>(i), l_outer_field_2_obj.get<int32_t>(i));
                CORE_ALWAYS_ASSERT_EQ(src_arr.get<int32_t>(i), dst_arr.get<int32_t>(i));
            }
            CORE_ALWAYS_ASSERT(not ErrorContext::has_error());
        }
    }
