...
class StdOutCallback : public llvm_proxy::object::Callback {
    using object_t = typename llvm_proxy::object::Callback::object_t;
private:
...
int32_t main(int32_t argc, char **argv) {
    ...
    CORE_ASSERT(pkg_module.has_error())
    CORE_ASSERT(fn.has_error())
    CORE_ASSERT(l_module.has_error())
    CORE_ASSERT(int32_type.has_error())
    CORE_ASSERT(void_type.has_error())
    ...
    CORE_INFO << " MAIN module name: [" << l_module.name() << "]";
    ...
    CORE_ASSERT(fn_section.has_error())
    CORE_ASSERT(l_fn_body.has_error())
#if 1
    ...
    CORE_ASSERT(not CodeSectionContext::current_section().is_sealed());
    CORE_ASSERT(not CodeSectionContext::current_section().is_commit());
    ...
    if (CodeSectionContext::current_section().is_valid()) {
        CORE_ASSERT(CodeSectionContext::current_section().is_sealed());
        CORE_ASSERT(CodeSectionContext::current_section().is_commit());
    }
    ...
    CODEGEN_LINE(CORE_INFO << " Module m : " << l_module.name())
    CODEGEN_LINE(CORE_INFO << " Function fn : " << fn.name())
    CODEGEN_LINE(CORE_INFO << " CodeSection: " << fn_section.name())
    CODEGEN_LINE(CORE_INFO << " Type name: " << int32_type.short_name())
    ErrorContext::print(std::cout, 5);
    CORE_ASSERT(not fn.has_error())
    CORE_ASSERT(not l_module.has_error())
...
#if 0
    {

        {
            ...
            for (int32_t i = 0; i != 10; ++i) {
                ...
                CORE_ALWAYS_ASSERT(res5 == fn(&args));
            }
            CORE_INFO << "sample_fn_name passed all tests";
        }
        {
            ...
            for (int32_t i = 0; i != 10; ++i) {
                ...
                CORE_ALWAYS_ASSERT(test_value_fwd_2 == fn(&args));
            }
            CORE_INFO << "foo_if_else passed all tests";
        }
        // TODO{vibhanshu}: add unit-test for foo-lop also
        {
            ...
            for (int32_t i = 0; i != 10; ++i) {
                ...
                CORE_ALWAYS_ASSERT(arr[0] == (i + 100));
                fn_load_args_struct args{arg_a, arg_b};
                CORE_ALWAYS_ASSERT(entry == fn(&args));
                CORE_ALWAYS_ASSERT(arr[0] == 1987);
            }
            CORE_INFO << "foo_loop_v2 passed all tests";
        }
    }
#endif
