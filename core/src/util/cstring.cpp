...
// find_result_t
//
find_result_t::find_result_t(ptrdiff_t i) : m_i{shrink(i)} {
}
...
CString::CString(const char* s, uint32_t sz) : m_s{s}, m_size{sz} {
    CORE_ASSERT(meta::bool_if(s == nullptr, sz == 0));
}

CString::CString(const std::string_view& s) : m_s{s.data()}, m_size{shrink(s.size())} {
}

CString::CString(const std::string& s) : m_s{s.data()}, m_size{shrink(s.size())} {
}
...
const char* CString::end() const {
    CORE_ASSERT(meta::bool_if(m_s == nullptr, m_size == 0));
    return m_s + m_size;
}

void CString::pop_front(uint32_t num_chars) {
    CORE_ASSERT(num_chars <= m_size);
    m_s += num_chars;
    m_size -= num_chars;
}
...
void CString::set_end(const char* ptr) {
    CORE_ASSERT(ptr != nullptr);
    CORE_ASSERT(ptr >= m_s);
    m_size = shrink(std::distance(m_s, ptr));
}

bool CString::empty() const {
    CORE_ASSERT(meta::bool_if(m_size > 0, m_s != nullptr));
    return m_size == 0;
}
...
CString CString::from_range(const char* begin, const char* end) {
    CORE_ASSERT(begin <= end);
    return CString{begin, shrink(end - begin)};
}
...
CString CString::from_pointer(const char* p, uint32_t max_size) {
    ...
    uint32_t len = shrink(::strnlen(p, max_size));
...
CString operator""_cs(const char* s, size_t len) {
    return CString::from_pointer(s, shrink(len));
}
