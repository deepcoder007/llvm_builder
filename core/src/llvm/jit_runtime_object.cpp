...
public:
    explicit Impl(const RuntimeStruct &parent)
        : m_parent{parent} {
        CORE_ASSERT(not m_parent.has_error());
...
public:
    ...
    T* get_field_location(const RuntimeField& field) const {
        CORE_ASSERT(is_instance_of(field.struct_def()));
        int32_t l_offset = field.offset();
        char* l_field_ptr = M_get_buf((uint32_t)l_offset);
        CORE_ASSERT(l_field_ptr  != nullptr);
    ...
    void add_object(const std::string& field_name, uint64_t field_addr, const RuntimeObject& o) {
        CORE_ASSERT(field_addr != 0);
        CORE_ASSERT(not o.has_error());
        CORE_ASSERT(o.is_frozen());
...
void RuntimeObject::print(std::ostream &os) const {
    os << "{RuntimeObject:";
    if (m_impl) {
        m_impl->print(os);
    } else {
...
public:
    explicit Impl(runtime_type_t element_type, uint32_t size)
        ...
        CORE_ASSERT(m_element_size != std::numeric_limits<uint32_t>::max())
        ...
        if (is_pointer()) {
            if (m_element_type == runtime_type_t::pointer_struct) {
                ...
                CORE_ASSERT(m_element_type == runtime_type_t::pointer_array);
...
public:
    ...
    void set_object(uint32_t i, const RuntimeObject& v) {
        CORE_ASSERT(m_element_type == runtime_type_t::pointer_struct);
        CORE_ASSERT(m_array_objects != nullptr);
        CORE_ASSERT(i < m_size);
        m_array_objects[i] = v;
    }
    void set_array(uint32_t i, const RuntimeArray& v) {
        CORE_ASSERT(m_element_type == runtime_type_t::pointer_array);
        CORE_ASSERT(m_array2_objects != nullptr);
        CORE_ASSERT(i < m_size);
...
void RuntimeArray::print(std::ostream& os) const {
    os << "{RuntimeArray:";
    if (m_impl) {
        os << m_impl->num_elements();
        os << ":";
        m_impl->log_values(os);
    } else {
        os << "<NULL>";
    }
    os << "}";
...
public:
    Impl(const RuntimeStruct& parent,
        ...
        CORE_ASSERT(not m_parent.has_error())
        CORE_ASSERT(m_idx != std::numeric_limits<int32_t>::max())
        CORE_ASSERT(m_offset != std::numeric_limits<int32_t>::max())
        CORE_ASSERT(not m_name.empty());
        if (is_struct_pointer()) {
            CORE_ASSERT(type.is_pointer());
            CORE_ASSERT(type.base_type().is_struct());
    ...
    void log_values(std::ostream& os, void* data) const {
        if (data != nullptr) {
            ...
            case runtime_type_t::pointer_struct: {
                            ...
                            CORE_ASSERT(not m_underlying_struct.has_error());
...
void RuntimeField::print(std::ostream& os) const {
    os << "{RuntimeField:";
    if (m_impl) {
        m_impl->print(os);
    } else {
...
runtime_type_t RuntimeField::get_type(const TypeInfo& type) {
    ...
    if (type.is_boolean()) {
        ...
        if (type.is_signed_integer()) {
            ...
            CORE_ASSERT(type.is_unsigned_integer());
            if (l_size == 8) {
...
public:
    explicit Impl(const RuntimeStruct& parent, const TypeInfo &type)
        ...
        for (uint32_t i = 0; i != l_num_fields; ++i) {
            ...
            CORE_ASSERT(l_field.idx() == i);
            CORE_DEBUG(auto it = ) m_fields.try_emplace(field_name, parent, l_field.idx(), l_field.offset(), field_name, l_field.type(), RuntimeField::construct_t{});
            CORE_ASSERT(it.second);
            m_field_names.emplace_back(field_name);
        }
        CORE_ASSERT(num_fields() == (int32_t)type.num_elements());
    }
    ~Impl() = default;
public:
    const std::string &name() const {
    ...
    RuntimeObject mk_object(const RuntimeStruct& parent) const {
        ...
        CORE_ASSERT(l_object.m_impl);
        CORE_ASSERT(not l_object.has_error());
        return l_object;
    }
    ...
    void print(std::ostream& os) const {
        os << "name=" << m_name << ", fields={" << num_fields() << "->";
...
void RuntimeEventFn::print(std::ostream &os) const {
    os << "{RuntimeEventFn:";
    if (m_impl) {
        m_impl->print(os);
    } else {
...
public:
    ...
    void add_struct(const RuntimeNamespace& parent, const TypeInfo &struct_type) {
        ...
        CORE_ASSERT(struct_type.is_struct())
        CORE_ASSERT(is_global());
        CORE_ASSERT(not is_bind());
...
RuntimeNamespace::RuntimeNamespace(JustInTimeRunner &runner, const std::string &ns, construct_t)
  : BaseT{State::VALID} {
    CODEGEN_FN
    CORE_ASSERT(not runner.has_error());
...
void RuntimeNamespace::print(std::ostream& os) const {
    os << "{RuntimeNamespace:";
    if (m_impl) {
        m_impl->print(os);
    } else {
