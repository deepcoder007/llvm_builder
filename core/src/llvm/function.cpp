...
void FnContext::print(std::ostream& os) const {
    os << "{FnContext: type=" << m_type << "}";
}
...
public:
    explicit Impl(const LinkSymbolName& symbol_name
        ...
        CORE_ASSERT(not m_fn_name.empty());
        CORE_ASSERT(not m_return_type.has_error());
        CORE_ASSERT(not m_context.has_error());
        CORE_ASSERT(is_valid());
        CORE_ASSERT(not m_link_symbol.has_error());
...
public:
    ...
    ValueInfo call_fn(const Function& parent, const ValueInfo& arg) const {
        ...
        CORE_ASSERT(Module::Context::has_value());
        Module& l_current_module = Module::Context::value();
        CORE_ASSERT(not l_current_module.has_error());
        llvm::Module* l_raw_module = l_current_module.native_handle();
        CORE_ASSERT(l_raw_module != nullptr);
    ...
    void declare_fn(const Module& src_mod, const Module& dst_mod) const {
        CORE_ASSERT(is_valid());
        CORE_ASSERT(not src_mod.has_error())
        CORE_ASSERT(not dst_mod.has_error());
        if (&dst_mod != &m_parent) {
            M_mk_fn(dst_mod);
        }
    }
...
private:
    static llvm::FunctionType* M_mk_fn_type(TypeInfo return_type, const FnContext& context) {
        CODEGEN_FN
        CORE_ASSERT(not return_type.has_error());
        ...
        CORE_ASSERT(l_fn_type != nullptr);
        return l_fn_type;
    }
    llvm::Function* M_mk_fn(const Module& dest_mod) const {
        CODEGEN_FN
        CORE_ASSERT(not dest_mod.has_error());
...
void Function::verify() {
    ...
    if (std::shared_ptr<Impl> ptr = m_impl.lock()) {
        ...
        CODEGEN_PUSH_ERROR(FUNCTION, "function already deleted");
        M_mark_error();
        CORE_ABORT("function is a null object");
    }
}
