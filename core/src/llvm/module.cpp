...
#undef DECL_DEL_TYPE
    ...
    void init(Cursor& parent) {
        CODEGEN_FN
        CORE_ASSERT(not parent.has_error());
        CursorPtr ptr{parent};
        CORE_ASSERT(ptr.is_valid());
        {
            llvm::Type* l_type = llvm::Type::getVoidTy(m_context);
            CORE_ASSERT(l_type->isVoidTy());
...
private:
    ...
    Module main_module() const {
        CODEGEN_FN
        CORE_ASSERT(is_bind_called());
        CORE_ASSERT(is_valid());
        CORE_ASSERT(m_main_module.is_init());
        return m_main_module;
    }
    Module gen_module(const std::weak_ptr<Impl>& ptr) {
        CORE_ASSERT(is_valid());
        CORE_ASSERT(is_bind_called());
        const std::string l_mod_name = CORE_CONCAT << m_name << "_" << m_modules.size();
    ...
    Function mk_function(FunctionImpl&& fn_impl) {
        ...
        CORE_ASSERT(is_valid());
        CORE_ASSERT(fn_impl.is_valid());
        FunctionImpl& impl =  m_func_list.emplace_back(std::move(fn_impl));
        CORE_ASSERT(not fn_impl.is_valid());
...
#undef DECL_MK_TYPE
    TypeInfo mk_type_array(TypeInfo element_type, uint32_t num_elements, CursorPtr parent) {
        ...
        CORE_ASSERT(num_elements != 0);
        CORE_ASSERT(element_type.is_valid_struct_field());
    ...
    TypeInfo mk_type_struct(const std::string& name, const std::vector<member_field_entry>& element_list, bool is_packed, CursorPtr parent) {
        ...
        CORE_ASSERT(is_valid());
        CORE_ASSERT(not name.empty());
        CORE_ASSERT(element_list.size() > 0);
        CORE_ASSERT(not is_bind_called());
        for ([[maybe_unused]] const member_field_entry& l_entry : element_list) {
            CORE_ASSERT(l_entry.is_valid());
            CORE_ASSERT(l_entry.type().is_valid_struct_field());
        }
...
private:
    Module M_gen_module(const std::string& mod_name, const std::weak_ptr<Impl>& ptr) {
        ...
        CORE_ASSERT(is_valid());
        CORE_ASSERT(is_bind_called());
        CORE_ASSERT(not mod_name.empty());
        CORE_ASSERT(not ptr.expired());
        CORE_ASSERT(not Module::Context::has_value());
...
llvm::LLVMContext& CursorPtr::ctx() {
    if (std::shared_ptr<Impl> ptr = m_impl.lock()) {
        ...
        CORE_ABORT(CORE_CONCAT << "No cursor found");
    }
}
...
void Cursor::print(std::ostream& os) const {
    if (m_impl) {
        os << "{Cursor:" << CursorPtr{const_cast<Cursor&>(*this)}.name() << "}";
    } else {
        os << "{Cursor:NULL}";
    }
}
...
Cursor Cursor::null() {
    ...
    CORE_ASSERT(s_null_cursor.has_error());
...
public:
    ...
    void register_symbol(const LinkSymbol &link_symbol) {
        CODEGEN_FN
        CORE_ASSERT(not link_symbol.has_error());
        CORE_ASSERT(not link_symbol.full_name().empty());
        CORE_ASSERT(not contains(link_symbol.full_name()));
        CORE_ASSERT(link_symbol.is_valid());
        m_public_symbols.emplace_back(link_symbol);
    }
    void init_standard() {
        CODEGEN_FN
        CORE_ASSERT(not is_init());
        llvm::DataLayout l_data_layout = CursorContextImpl::get_host_data_layout();
        CORE_ASSERT(m_raw_module);
    ...
    // TODO{vibhanshu}: this is a high level functionality, need not be part of core framework
    void add_struct_definition(Module& parent, const TypeInfo &struct_type) {
        CODEGEN_FN
        CORE_ASSERT(not parent.has_error());
        CORE_ASSERT(not struct_type.has_error());
    ...
    void write_llvm_output(const std::string& suffix) const {
        ...
        const std::string stage_file_path = [suffix, this]() -> std::string {
            const std::string l_base_dir = SysUtil::cache_dir();
            if (suffix.empty()) {
                return CORE_CONCAT << l_base_dir << "/" << name() << ".ll";
            } else {
                return CORE_CONCAT << l_base_dir << "/" << name() << "_" << suffix << ".ll";
            }
        } ();
...
void Module::register_symbol(const LinkSymbol& link_symbol) {
    ...
    if (not link_symbol.is_valid()) {
        CODEGEN_PUSH_ERROR(MODULE, "link symbol not valid:" << link_symbol);
        M_mark_error();
        return;
    }
    if (std::shared_ptr<Impl> ptr = m_impl.lock()) {
        if (ptr->contains(link_symbol.full_name())) {
            CODEGEN_PUSH_ERROR(MODULE, "symbol already exists:" << link_symbol);
            M_mark_error();
            return;
        }
...
Module Module::null() {
    ...
    CORE_ASSERT(s_module.has_error());
    return s_module;
}
...
void PackagedModule::print(std::ostream& os) const {
    os << "{PackagedModule:" << name() << "}";
}
