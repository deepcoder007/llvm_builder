...
public:
    explicit Impl(const std::string& name, branch_type type, Function fn, IfElseCond& parent)
        ...
        CORE_ASSERT(m_type == branch_type::br_then or m_type == branch_type::br_else);
        m_section = fn.mk_section(m_name);
    }
    ~Impl() {
        if (m_section.is_open()) {
            CORE_ASSERT(m_section.is_sealed());
        }
    }
public:
    bool is_open() const {
    ...
    void enter() {
        CODEGEN_FN
        CORE_ASSERT(not m_section.is_sealed());
        CORE_ASSERT(not m_section.is_commit());
        m_section.enter();
    ...
    void exit() {
        ...
        CORE_ASSERT(m_section.is_sealed());
        CORE_ASSERT(m_section.is_commit());
...
IfElseCond::IfElseCond(const std::string &name, const ValueInfo &value)
    : m_name{name}, m_value{value}, m_parent{CodeSectionContext::current_section()},
      m_then_branch{CORE_CONCAT << m_name << ".then", branch_type::br_then,
                    CodeSectionContext::current_function(), *this},
      m_else_branch{CORE_CONCAT << m_name << ".else", branch_type::br_else, CodeSectionContext::current_function(), *this},
      m_post_if_branch{CodeSectionContext::current_function().mk_section(CORE_CONCAT << name << ".post")} {
    if (m_name.empty()) {
...
void IfElseCond::bind() {
    ...
    if (m_is_inside_branch_type == branch_type::invalid) {
        CORE_ASSERT(CodeSectionContext::is_current_section(m_parent));
...
void IfElseCond::enter_branch(c_construct, branch_type type) {
    if (m_is_inside_branch_type == branch_type::invalid) {
        ...
        if (type == branch_type::br_then) {
            CORE_ASSERT(m_then_branch.is_open());
            CORE_ASSERT(not m_then_branch.is_sealed());
        } else {
            CORE_ASSERT(type == branch_type::br_else);
            CORE_ASSERT(m_else_branch.is_open());
            CORE_ASSERT(not m_else_branch.is_sealed());
        }
    } else {
...
void IfElseCond::exit_branch(c_construct, [[maybe_unused]] branch_type type) {
    if (m_is_inside_branch_type != branch_type::invalid) {
        CORE_ASSERT(m_is_inside_branch_type == type);
