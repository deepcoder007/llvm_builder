...
struct TypeInfo::derived_info_t : meta::noncopyable {
    ...
    public:
        ...
        explicit full_field_info_t(uint32_t offset, const std::string& name, const TypeInfo& type)
            ...
            CORE_ASSERT(offset != std::numeric_limits<uint32_t>::max())
            CORE_ASSERT(not name.empty())
            CORE_ASSERT(not type.has_error())
            CORE_ASSERT(not type.has_error());
        }
    ...
    public:
        ...
        std::string short_name() const {
            return CORE_CONCAT << "{" << BaseT::name() << ":" << BaseT::type().short_name() << "}";
        }
        ...
        void print(std::ostream& os) const {
            os << "{name=" << BaseT::name() << ", type=" << BaseT::type() << "}";
        }
        OSTREAM_FRIEND(full_field_info_t)
    };
...
public:
    explicit derived_info_t(type_id_t type, TypeInfo base_type)
        : m_type{type} {
        CORE_ASSERT(type == type_id_t::pointer_t);
        CORE_ASSERT(not base_type.has_error());
    ...
    explicit derived_info_t(type_id_t type, TypeInfo element_type, uint32_t num_elements)
        ...
        CORE_ASSERT(is_array() or is_vector());
        CORE_ASSERT(m_num_elements > 0);
        CORE_ASSERT(not element_type.has_error());
    ...
    explicit derived_info_t(type_id_t type
        ...
        : m_type{type}, m_name{name}, m_num_elements{meta::shrink(field_types.size())} {
        CORE_ASSERT(type == type_id_t::struct_t);
        CORE_ASSERT(not m_name.empty());
        CORE_ASSERT(field_types.size() > 0);
        ...
        CORE_ASSERT(field_types.size() == field_offsets.size());
        const uint32_t l_num_fields = meta::shrink(field_types.size());
        for (uint32_t i = 0; i != l_num_fields; ++i) {
            ...
            CORE_ASSERT(it.second);
        }
        CORE_ASSERT(m_num_elements == m_type_list.size());
...
public:
    ...
    const TypeInfo& base_type() const {
        if (is_pointer() or is_array() or is_vector()) {
            ...
            CODEGEN_PUSH_ERROR(VALUE_ERROR, CORE_CONCAT << "can't define base-type of type:" << short_name())
    ...
    field_entry_t  operator [] (uint32_t i) const {
        ...
        if (is_struct() and i < m_type_list.size()) {
            ...
            CODEGEN_PUSH_ERROR(VALUE_ERROR, CORE_CONCAT << "unable to find field:" << i << " in struct:" << name())
    ...
    field_entry_t operator [] (const std::string& s) const {
        ...
        CODEGEN_PUSH_ERROR(VALUE_ERROR, CORE_CONCAT << "unable to find field:" << s << " in struct:" << name())
        return field_entry_t::null();
    }
    ...
    std::string short_name() const {
        if (is_pointer()) {
            return CORE_CONCAT << "ptr[" << m_type_list[0].type().short_name() << "]";
        } else if (is_array()) {
            return CORE_CONCAT << "array[" << m_type_list[0].type().short_name()
                << ":" << m_num_elements << "]";
        } else if (is_vector()) {
            return CORE_CONCAT << "vector[" << m_type_list[0].type().short_name()
                << ":" << m_num_elements << "]";
        } else {
            CORE_ASSERT(is_struct());
            std::stringstream os;
    ...
    void print(std::ostream &os) const {
        if (is_pointer()) {
            os << "{pointer:" << m_type_list[0] << "}";
        } else if (is_array()) {
            os << "{array:" << m_type_list[0]
            << ", num_elements:" << m_num_elements
            << "}";
        } else if (is_vector()) {
            os << "{vector:" << m_type_list[0]
            << ", num_elements:" << m_num_elements
            << "}";
        } else {
            CORE_ASSERT(is_struct());
...
void TypeInfo::field_entry_t::print(std::ostream& os) const {
    os << "{entry:name=" << m_name
        << ", offset=" << m_offset
        << ", type=" << m_type.short_name() << "}";
}
...
#define BINARY_OP_IMPL_FN(FN_NAME, SIGN_FN, UNSIGNED_FN, FLOAT_FN)      \
    llvm::Value* FN_NAME (llvm::Value* lhs, llvm::Value* rhs, const std::string& op_name) { \
        CODEGEN_FN                                                      \
        CORE_ASSERT(lhs != nullptr)                                 \
        CORE_ASSERT(rhs != nullptr)                                 \
        ...
        if (is_vector()) {                                              \
            ...
            if (l_base_type.is_signed_integer()) {                      \
                ...
                CORE_ASSERT(l_base_type.is_unsigned_integer());         \
            ...
            if (is_signed_integer()) {                                  \
                ...
                CORE_ASSERT(is_unsigned_integer());                     \
                return l_builder. UNSIGNED_FN (lhs, rhs, op_name);      \
            }                                                           \
        } else if (is_boolean()) {                                      \
            return l_builder. UNSIGNED_FN(lhs, rhs, op_name);           \
...
public:
    explicit Impl(void_construct_t, CursorPtr cursor_impl, llvm::Type *type) : m_type{type}, m_cursor_impl{cursor_impl}, m_type_id{type_id_t::void_t} {
        CORE_ASSERT(m_type != nullptr);
        CORE_ASSERT(m_type->isVoidTy());
        CORE_ASSERT(m_type->getPrimitiveSizeInBits() == m_num_bytes);
        CORE_ASSERT(m_cursor_impl.is_valid());
    ...
    explicit Impl(int_construct_t, CursorPtr cursor_impl, llvm::Type* type, uint32_t num_bytes) : m_type{type}, m_cursor_impl{cursor_impl}, m_num_bytes{num_bytes} {
        CORE_ASSERT(m_type != nullptr);
        CORE_ASSERT(m_type->isIntegerTy());
        CORE_ASSERT(m_cursor_impl.is_valid());
        if (m_type->getPrimitiveSizeInBits() == 1) {
            CORE_ASSERT(m_num_bytes == 1);
    ...
    explicit Impl(float_construct_t, CursorPtr cursor_impl, llvm::Type* type, uint32_t num_bytes)
        ...
        CORE_ASSERT(m_type != nullptr);
        CORE_ASSERT(m_type->isFloatTy() or m_type->isDoubleTy());
        CORE_ASSERT((m_type->getPrimitiveSizeInBits() / 8) == m_num_bytes);
        CORE_ASSERT(m_cursor_impl.is_valid());
        object::Counter::singleton().on_new(object::Callback::object_t::TYPE, (uint64_t)this, short_name());
    }
    explicit Impl(pointer_construct_t, CursorPtr cursor_impl, llvm::Type* type, uint32_t num_bytes, const TypeInfo& base_type)
        ...
        CORE_ASSERT(m_type != nullptr);
        CORE_ASSERT(m_type->isPointerTy());
        CORE_ASSERT(m_num_bytes == sizeof(int*));
        CORE_ASSERT(not base_type.has_error());
        CORE_ASSERT(m_cursor_impl.is_valid());
    ...
    explicit Impl(int_construct_t, CursorPtr cursor_impl, llvm::IntegerType* type, bool is_signed)
        ...
        CORE_ASSERT(m_cursor_impl.is_valid());
        m_type_id = type_id_t::int_t;
        CORE_ASSERT(m_num_bytes == 1 or m_num_bytes == 2 or m_num_bytes == 4 or m_num_bytes == 8);
        object::Counter::singleton().on_new(object::Callback::object_t::TYPE, (uint64_t)this, short_name());
    }
    ...
    explicit Impl(vector_construct_t, CursorPtr cursor_impl, TypeInfo element_type, uint32_t vector_size)
        ...
        CORE_ASSERT(not element_type.has_error());
        CORE_ASSERT(element_type.is_scalar());
        CORE_ASSERT(vector_size > 0);
        CORE_ASSERT(m_cursor_impl.is_valid());
    ...
    explicit Impl(struct_construct_t, CursorPtr cursor_impl, const std::string &name,
        ...
        CORE_ASSERT(m_cursor_impl.is_valid());
        CORE_ASSERT(field_types.size() > 0);
        CORE_ASSERT(not m_cursor_impl.is_bind_called());
        ...
        for (const member_field_entry& l_info : field_types) {
            CORE_ASSERT(l_info.is_valid());
            const TypeInfo& l_type = l_info.type();
            CORE_ASSERT(l_type.is_valid())
            CORE_ASSERT(l_type.is_scalar() or l_type.is_pointer());
...
public:
    ...
    std::string short_name() const {
        ...
        if (is_struct() or is_vector() or is_array() or is_pointer()) {
            ...
            if (is_signed_integer()) {
                return CORE_CONCAT << "int" << (m_num_bytes * 8);
            } else {
                CORE_ASSERT(is_unsigned_integer());
                return CORE_CONCAT << "uint" << (m_num_bytes * 8);
            }
        } else if (is_float()) {
            return CORE_CONCAT << "float" << (m_num_bytes * 8);
        } else {
            return CORE_CONCAT << "<UNKNOWN SHORT NAME>";
        }
    }
    [[gnu::noinline]]
    void print(std::ostream &os) const {
        os << "{Type:value:" << m_type
            ...
            << ", is_struct:" << is_struct();
        if (m_derived_info) {
            os << ", derived_info:" << *m_derived_info;
        }
        os << "}";
    }
    [[gnu::noinline]]
    ...
    bool check_sync(const ValueInfo &value) const {
        CODEGEN_FN
        CORE_ASSERT(not value.has_error());
        CORE_ASSERT(m_type != nullptr);
        ...
        if (is_boolean()) {
            ...
            if (l_res) {
                CORE_ASSERT(llvm::ArrayType::classof(m_type));
                CORE_ASSERT(llvm::ArrayType::classof(l_type));
            ...
            if (l_res) {
                CORE_ASSERT(llvm::FixedVectorType::classof(m_type));
                CORE_ASSERT(llvm::FixedVectorType::classof(l_type));
            ...
            if (l_res) {
                CORE_ASSERT(llvm::StructType::classof(m_type));
                CORE_ASSERT(llvm::StructType::classof(l_type));
...
public:
    ...
    llvm::Value *M_fixed_point_type_cast(const ValueInfo &src_value,
        ...
        CODEGEN_FN
        CORE_ASSERT(not src_value.has_error());
        CORE_ASSERT(m_type != nullptr and not src_value.has_error());
    ...
    llvm::Value *M_fixed_to_floating_type_cast(const ValueInfo &src_value,
        ...
        if (src_value.type().is_signed_integer()) {
            ...
            CORE_ASSERT(src_value.type().is_unsigned_integer());
    ...
    llvm::Value* type_cast(const ValueInfo &src_value,
        ...
        if (src_type.is_integer() and is_integer()) {
            ...
            llvm::Value* src = src_value.native_value();
            llvm::IRBuilder<>& l_builder = CursorContextImpl::builder();
            CORE_ASSERT(src->getType()->isIntegerTy());
            return l_builder.CreateZExtOrTrunc(src, m_type, op_name);
        } else {
...
auto TypeInfo::null() -> TypeInfo& {
    ...
    CORE_ASSERT(s_null_type.has_error());
    return s_null_type;
}
...
TypeInfo TypeInfo::from_raw(llvm::Type* l_raw_type) {
    CODEGEN_FN
    CORE_ASSERT(l_raw_type != nullptr);
    if (l_raw_type->isVoidTy()) {
        ...
        CORE_ABORT("Struct Type yet to be implemented");
    } else {
...
std::string LinkSymbolName::M_full_name() const {
    if (has_error()) {
        ...
        return CORE_CONCAT << namespace_name() << s_ns_separator << short_name();
    }
}
...
void LinkSymbolName::print(std::ostream& os) const {
    os << "Symbol:[" << m_namespace << ":" << m_name << "]";
}

LinkSymbolName LinkSymbolName::null() {
    static LinkSymbolName s_null{};
    CORE_ASSERT(s_null.has_error());
    return s_null;
}
...
bool LinkSymbol::operator == (const LinkSymbol& o) const {
    ...
    CORE_ASSERT(meta::bool_if(r, m_ctype == o.m_ctype and m_type == o.m_type));
...
void LinkSymbol::print(std::ostream& os) const {
    // TODO{vibhanshu}: add more formats for printing of linksymbol
    os << "Symbol:[" << full_name() << "]";
}
...
TypeInfoImpl::TypeInfoImpl(void_construct_t, CursorPtr cursor_impl, llvm::Type* type) {
    CORE_ASSERT(cursor_impl.is_valid());
