...
void TagInfo::print(std::ostream& os) const {
    os << "{TagInfo:[";
    separator_t sep{"|"_cs};
...
void ValueInfo::print(std::ostream& os) const {
    os << "{Value:type=" << m_type_info.short_name() << ", tag=" << m_tag_info << "}";
}
...
ValueInfo ValueInfo::load_vector_entry(uint32_t i) const {
    ...
    return load_vector_entry(i, CORE_CONCAT << "load_vector_" << i);
}
...
ValueInfo ValueInfo::store_vector_entry(uint32_t i, ValueInfo value) const {
    ...
    return store_vector_entry(i, value, CORE_CONCAT << "store_vector_" << i);
}
...
ValueInfo ValueInfo::load_vector_entry(const ValueInfo& idx_v) const {
    ...
    return M_load_vector_entry(idx_v.native_value(), CORE_CONCAT << "load_vector_idx");
}
...
ValueInfo ValueInfo::store_vector_entry(const ValueInfo& idx_v, ValueInfo value) const {
    ...
    return M_store_vector_entry(idx_v.native_value(), value, CORE_CONCAT << "load_vector_idx");
}
...
auto ValueInfo::null() -> ValueInfo {
    ...
    static ValueInfo s_null_value{};
    CORE_ASSERT(s_null_value.has_error());
    return s_null_value;
}
...
ValueInfo ValueInfo::mk_pointer(TypeInfo type, const std::string& name) {
    ...
    if (CursorContextImpl::has_value()) {
        ...
        CORE_ASSERT(l_inst != nullptr);
        [[maybe_unused]] llvm::PointerType* ptr_type = l_inst->getType();
        CORE_ASSERT(ptr_type != nullptr);
        CORE_ASSERT(ptr_type->isLoadableOrStorableType(type.native_value()));
...
ValueInfo ValueInfo::mk_null_pointer() {
    ...
    CORE_ASSERT(llvm::PointerType::classof(l_ptr.native_value()));
...
ValueInfo ValueInfo::calc_struct_field_offset(TypeInfo type, ValueInfo idx) {
    ...
    if (CursorContextImpl::has_value()) {
        ...
        for (int32_t i = 0; i != l_num_fields; ++i) {
            ...
            ValueInfo l_match_idx = l_curr_iter_idx.equal(idx, CORE_CONCAT << "eq_field_idx_" << i);
            ValueInfo l_offset_curr = ValueInfo::from_constant(static_cast<int32_t>(type[i].offset()));
            l_offset_result = l_match_idx.cond(l_offset_curr, l_offset_result, CORE_CONCAT << "field_offset_" << i);
        }
...
ValuePtrInfo::ValuePtrInfo(const ValueInfo& v)
    ...
    CORE_ASSERT(not v.has_error());
    CORE_ASSERT(m_raw_value != nullptr);
    CORE_ASSERT(m_orig_type.is_pointer());
