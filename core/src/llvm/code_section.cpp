...
public:
    explicit VariableContext(std::unique_ptr<VariableContext>& parent)
        ...
        if (m_parent) {
            CORE_ASSERT(m_parent->is_active());
            m_parent->disable();
        }
        enable();
    }
    ~VariableContext() {
        CORE_ASSERT(not m_parent);
    }
public:
    ...
    void mk_ptr(const std::string& name, const TypeInfo& type, const ValueInfo& default_value) {
        ...
        CORE_ASSERT(not default_value.has_error())
        CORE_ASSERT(is_active());
...
public:
    ...
    void enter() {
        ...
        CORE_ASSERT(not is_open());
        CORE_ASSERT(not is_sealed());
        CORE_ASSERT(not is_commit());
        m_cursor_impl.builder().SetInsertPoint(m_basic_block);
        m_is_open = true;
    }
    void exit() {
        CORE_ASSERT(is_open());
        CORE_ASSERT(is_sealed());
        CORE_ASSERT(not is_commit());
    ...
    void set_return_value(ValueInfo value) {
        ...
        CORE_ASSERT(not is_sealed());
        CORE_ASSERT(not is_commit());
        CORE_ASSERT(value.type() == m_fn.return_type());
    ...
    void jump_to_section(Impl& s) {
        ...
        CORE_ASSERT(not is_commit());
        CORE_ASSERT(this != &s);
        CORE_ASSERT(s.m_basic_block != nullptr);
...
void CodeSection::enter() {
    ...
    if (std::shared_ptr<Impl> ptr = m_impl.lock()) {
        ...
        if (not has_error()) {
            ...
            CORE_ASSERT(l_type_info.is_pointer() or l_type_info.is_scalar());
...
void CodeSection::print(std::ostream& os) const {
    os << "{CodeSection:" << name() << "}";
}
...
void CodeSectionContext::assert_no_context() {
    vec_t& l_vec = M_sections();
    if (l_vec.size() > 0) {
        CORE_INFO << "List of open sections:" << l_vec.size() << " - ";
        for (const CodeSection &l_sec : l_vec) {
            CORE_INFO << " > " << l_sec.name() << ":" << l_sec.is_sealed();
        }
        CORE_ABORT("sections open found");
    }
}
